...
xaz["⟨uᵢ′τᵅ′⟩"] = xaz["⟨uᵢ′τᵅ′⟩"].transpose(..., "i", "α")
xaz["∇ⱼτ̄ᵅ"]     = xaz["∇ⱼτ̄ᵅ"].transpose(..., "j", "α")
#---

#+++ Maybe normalize averages before starting the method
if normalize_tracers:
    #rms = np.sqrt((xaz["∇ⱼτ̄ᵅ"]**2).mean(("xC", "zC", "j")))
    rms = np.sqrt((xaz["⟨uᵢ′τᵅ′⟩"]**2).mean(("xC", "zC", "i")))

    xaz["∇ⱼτ̄ᵅ|ᴿ"]     = xaz["∇ⱼτ̄ᵅ"]     / rms
    xaz["⟨uᵢ′τᵅ′⟩|ᴿ"] = xaz["⟨uᵢ′τᵅ′⟩"] / rms
#---

#+++ Invert gradient matrix using a Pseudo-Inverse algorithm
from numpy.linalg import pinv
xaz = xaz.chunk(α=-1, i=-1, j=-1)
xaz["∇ⱼτ̄ᵅ|ᴿ ⁻¹"] = xr.apply_ufunc(pinv, xaz["∇ⱼτ̄ᵅ|ᴿ"],
                               input_core_dims = [["α", "j"]],
                               output_core_dims = [["j", "α"]],
                               exclude_dims = set(("α", "j")),
                               dask_gufunc_kwargs = dict(output_sizes = {'α': len(xaz.α), 'j': len(xaz.j)}),
                               dask = "parallelized",
                               )
...
if test_propagation:
    ...
    with ProgressBar():
        A1 = pinv(xaz["∇ⱼτ̄ᵅ|ᴿ"].isel(time=0, zC=0, xC=0).data)
        B1 = xaz["∇ⱼτ̄ᵅ|ᴿ ⁻¹"].isel(time=0, zC=0, xC=0)
        C1 = xaz["∇ⱼτ̄ᵅ|ᴿ"].isel(time=0, zC=0, xC=0)
...
xaz["∇ⱼτ̄ᵅ|ᴿ ⁻¹"] = xaz["∇ⱼτ̄ᵅ|ᴿ ⁻¹"].transpose(..., "α", "j")

xaz["Rᵢⱼ"] = -xr.apply_ufunc(np.matmul, xaz["⟨uᵢ′τᵅ′⟩|ᴿ"], xaz["∇ⱼτ̄ᵅ|ᴿ ⁻¹"],
...
if test_propagation:
    ...
    with ProgressBar():
        A2 = -xaz["⟨uᵢ′τᵅ′⟩|ᴿ"].isel(time=0, zC=0, xC=0) @ xaz["∇ⱼτ̄ᵅ|ᴿ ⁻¹"].isel(time=0, zC=0, xC=0)
        B2 =  xaz["Rᵢⱼ"].isel(time=0, zC=0, xC=0)
